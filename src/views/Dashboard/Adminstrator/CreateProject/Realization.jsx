...
function Realization(e) {
  const {realization,setrealization,setImagesR,ImagesR} = useStateContext()
  const [info,setinfo] = useState({title:"",desc:"",image:""})
  const [OverviewList,setOverviewList] = useState(realization ? realization.title.length>0 ? realization:{title:[],desc:[]}:{title:[],desc:[]})
  const [imagesRea,setimagesRea] = useState(ImagesR ? ImagesR.length>0 ? ImagesR:[]:[])
  ...
  const handleConfirme = ()=>{
    if(e.p){
      setrealization(infoEdit)
    }else{
      setrealization(OverviewList)
      setImagesR(imagesRea)
    }
  }
  const handleEdit = ()=>{
